...
@onready var item_data_path: String = "res://Data/Item_data.json"
@onready var mask_data_path: String = "res://Data/Mask_data.json"

# Called when the node enters the scene tree for the first time.
func _ready():
	load_data(item_data_path)
	load_mask_data(mask_data_path)
	set_grid_data()
	set_mask_data()
...
func load_mask_data(path: String) -> void:
	if not FileAccess.file_exists(path):
		print("Mask Data file not found")
	var mask_data_file = FileAccess.open(path, FileAccess.READ)
	mask_data = JSON.parse_string(mask_data_file.get_as_text())
	mask_data_file.close()
	#print(mask_data)	#check value
...
func set_mask_data() -> void:
	for mask in mask_data.keys():
		var temp_grid_array := []
		for point in mask_data[mask]["Grid"].split("/"):
			temp_grid_array.push_back(point.split(","))
		mask_grid_data[mask] = temp_grid_array
	#print(item_grid_data)	#check values
